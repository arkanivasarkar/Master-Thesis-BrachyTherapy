...
def extractData(InputDicomFolder, OutputLocation):
    ...
    # ds = pydicom.dcmread(r"C:\Users\arkaniva\Downloads\US.1.2.528.1.1007.235.5.1.20250520.153003.1153988799.dcm")
    # mask = ds.pixel_array.astype(np.float32)

    grid_mask_file = nib.load(r"C:\Users\arkaniva\Downloads\Segmentation-Segment_1-label.nii")
    mask = np.squeeze(grid_mask_file.get_fdata()).T
    mask = mask.astype(np.float32)

    # Normalize and prepare mask edges once
    mask_edges = feature.canny(mask / np.max(mask), sigma=1.0)
    ...
    for i in range(pixel_matrix.shape[0]):
        slice_img = pixel_matrix[i].astype(np.float32)
        slice_img /= np.max(slice_img)

        slice_edges = feature.canny(slice_img, sigma=1.0)


        # Compute shift
        # shift, _, _ = phase_cross_correlation(slice_edges, mask_edges, upsample_factor=10)

        shift, error, _ = phase_cross_correlation(slice_edges, mask_edges, upsample_factor=10)
        print(error)

        mask_aligned = np.roll(mask, shift.astype(int), axis=(0, 1))

        

        pixel_matrix[i] = pixel_matrix[i]*np.logical_not(mask_aligned)

        print(pixel_matrix.dtype)


        indices = np.argwhere(mask_aligned == 1)
        ...
        for _ in range(3):
            for col,row in indices:
                for delta_c,delta_r in neighbourhood:
                    kernel_center = [col + delta_c, row + delta_r]

                    # plt.imshow(pixel_matrix[i],cmap='gray')
                    # plt.plot(kernel_center[1],kernel_center[0],'r*')
                    # plt.show()

                    # print(kernel_center)

                    # print(list(range(kernel_center[0]-1,kernel_center[0]+2)))
                    # print(list(range(kernel_center[1]-1,kernel_center[1]+2)))

                    # print(pixel_matrix[i][kernel_center[0], kernel_center[1]])
                    
                    patch = pixel_matrix[i][kernel_center[0]-1:kernel_center[0]+2, kernel_center[1]-1:kernel_center[1]+2]
                    # print(patch.shape)
                    ...
                    pixel_matrix[i][kernel_center[0],kernel_center[1]] = ((1/9)*patch.sum()).astype(pixel_matrix.dtype)
        ...
        # plt.plot(indices[0][1], indices[0][0],'r*')
        # plt.show()

        # Apply shift to mask using affine transform
        # tform = transform.AffineTransform(translation=shift[::-1])  # (y, x) -> (x, y)
        # aligned = transform.warp(mask, tform, mode='constant', cval=0, preserve_range=True)
      

        # plt.imshow(aligned,cmap='Reds', alpha=0.5)
        # plt.imshow(mask,cmap='Blues', alpha=0.5)
        # plt.imshow(pixel_matrix[i],cmap='gray')
        # plt.show()
    
    psnr_value = peak_signal_noise_ratio(m, pixel_matrix)
    ssim_value = structural_similarity(m, pixel_matrix)
